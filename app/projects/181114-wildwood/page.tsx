  ...
  return (
    <div className='h-auto max-w-full'>
      ...
      {images.map((image, index) => (
        <div key={index} className="mx-auto max-w-screen-xl" style={{ padding: '10px ', }}>
          <Image
            src={image.default}
            alt={`Image ${index + 1}`}
            className="mx-auto max-w-80vw sm:max-w-60vw md:max-w-40vw lg:max-w-30vw xl:max-w-25vw"
          />
        </div>
      ))}
    </div>
